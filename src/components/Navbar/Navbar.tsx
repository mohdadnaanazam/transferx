...
function NavLink({ href, name, path }: { href: string, name: string, path: string }) {
  ...
  return (
    <div className="relative group">
      <Link href={href} className="dark:text-gray-400 py-2 px-4 text-sm text-gray-900 dark:hover:text-white dark:hover:bg-accent hover:rounded tracking-wide transition-all duration-700 ease-in-out">
        {name}
      </Link>
      <div
        className={`absolute -bottom-4 left-1/2 transform -translate-x-1/2 w-full h-[2px] bg-white transition-all duration-700 ease-in-out ${
